data = readtable("C:/matlab/project/UNSW KD15/UNSW_NB15_testing-set.csv");
head(data)
cvp = cvpartition(data.proto,Holdout=0.2);
dataTrain = data(training(cvp),:);
dataValidation = data(test(cvp),:);
documentsTrain = preprocessText(dataTrain.attack_cat);
TTrain = categorical(dataTrain.proto);
classNames = unique(TTrain)
numObservations = numel(TTrain)
documentsValidation = preprocessText(dataValidation.attack_cat);
TValidation = categorical(dataValidation.proto);
